...
from metro_bike_share_forecasting.station_level.forecasting.features import (
...
def _active_feature_columns(frame: pd.DataFrame, config: StationLevelForecastConfig) -> list[str]:
    columns = STATION_FEATURE_COLUMNS.copy()
    if not config.include_category_feature and "station_category" in columns:
        columns.remove("station_category")
    if not config.include_cluster_feature and "cluster_label" in columns:
...
def _validation_split(training_data: pd.DataFrame, horizon: int) -> tuple[pd.DataFrame, pd.DataFrame]:
    cutoff = pd.to_datetime(training_data["date"]).max() - pd.Timedelta(days=horizon)
    train = training_data.loc[training_data["date"] <= cutoff].copy()
    valid = training_data.loc[training_data["date"] > cutoff].copy()
    if train.empty or valid.empty:
        return training_data.copy(), pd.DataFrame(columns=training_data.columns)
    return train, valid
...
def fit_deepar_model(
    ...
    encoded, metadata = _encode_features(train_frame, feature_columns)
    ...
    target = torch.tensor(train_frame["target"].astype(float).to_numpy(dtype=np.float32))
...
def predict_with_deepar(
    ...
    for forecast_date in forecast_dates:
        ...
        batch["implementation"] = "torch_deepar_like"
        batch["tuned"] = artifact.tuned
        batch["selected_params"] = str(artifact.selected_params)
        predictions.append(batch)
