...
from system_level.common.metrics import seasonal_naive_scale
...
def station_mase_scales(train_panel: pd.DataFrame, season_length: int) -> dict[str, float]:
    ...
    for station_id, station_frame in observed.groupby("station_id", sort=True):
        scales[str(station_id)] = seasonal_naive_scale(station_frame["target"], season_length)
    return scales
...
def expand_horizon_rows(
    ...
    scales = station_mase_scales(train_panel, config.mase_season_length)
...
def summarize_fold_metrics(scored_forecasts: pd.DataFrame) -> pd.DataFrame:
    if scored_forecasts.empty:
        return pd.DataFrame()
    scored = scored_forecasts.copy()
    if "implementation" not in scored.columns:
        scored["implementation"] = "unknown"
    scored["implementation"] = scored["implementation"].fillna("unknown")
    metrics = (
        scored.groupby(["model_name", "implementation", "fold_id", "horizon"], as_index=False)
        .agg(
            mae=("abs_error", "mean"),
