...
from system_level.common.validation import assert_known_future_feature_coverage
...
def build_known_future_features(date_frame: pd.DataFrame, config: SystemLevelConfig, external_features: pd.DataFrame) -> pd.DataFrame:
    ...
    if not external_features.empty:
        ...
        external_columns = [column for column in external_features.columns if column != "date"]
        assert_known_future_feature_coverage(known, external_columns)
    return known
