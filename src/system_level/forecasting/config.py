...
from system_level.common.validation import KNOWN_FUTURE_PREFIXES
...
class SystemLevelConfig:
    ...
    external_feature_prefixes: tuple[str, ...]
    initial_train_size: int
    step_size: int
    max_folds: int
    mase_season_length: int
...
def load_system_level_config(config_path: str | Path) -> SystemLevelConfig:
    ...
    return SystemLevelConfig(
        ...
        missing_target_strategy=str(input_cfg.get("missing_target_strategy", "zero_fill_with_flag")),
        external_feature_prefixes=tuple(feature_cfg.get("external_feature_prefixes", KNOWN_FUTURE_PREFIXES)),
        ...
        mase_season_length=int(backtest_cfg.get("mase_season_length", default_mase_season_length(input_cfg.get("frequency")))),
