...
from system_level.forecasting.models import MODEL_DIAGNOSTIC_COLUMNS
...
def write_interval_summary_report(
    ...
    recommendations = build_recommendation_table(point_summary)
    lines = [
    ...
    for _, recommendation in recommendations.iterrows():
        row = point_summary.loc[
            (point_summary["horizon"] == recommendation["horizon"])
            & (point_summary["model_name"] == recommendation["recommended_model"])
        ].iloc[0]
        lines.append(
            f"- `{recommendation['recommended_model']}` at h={int(recommendation['horizon'])}: "
            f"MAE={row['mean_mae']:.2f}, RMSE={row['mean_rmse']:.2f}, "
            f"MASE={row['mean_mase']:.3f}, Bias={row['mean_bias']:.2f}."
        )

    lines.extend(["", "## Interval Coverage"])
    for _, recommendation in recommendations.iterrows():
        subset = interval_summary.loc[
            (interval_summary["horizon"] == recommendation["horizon"])
            & (interval_summary["model_name"] == recommendation["recommended_model"])
        ]
        if subset.empty:
            continue
        row = subset.iloc[0]
        lines.append(
            f"- `{recommendation['recommended_model']}` at h={int(recommendation['horizon'])}: "
            f"coverage_80={row['coverage_80']:.3f}, coverage_95={row['coverage_95']:.3f}, "
