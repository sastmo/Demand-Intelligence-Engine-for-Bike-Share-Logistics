...
DEFAULT_OUTPUT_ROOT = Path("diagnosis/system_level/outputs")
...
def add_arguments(parser: argparse.ArgumentParser) -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        help="Anomaly detection method. The centered method is retrospective; the causal method is one-sided.",
    )
    return parser
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Run system-level forecasting diagnostics and save outputs into diagnosis/system_level/outputs."
    )
...
def load_dataset(args: argparse.Namespace) -> tuple[pd.DataFrame, str | None]:
...
def apply_optional_filters(frame: pd.DataFrame, time_col: str | None, args: argparse.Namespace) -> pd.DataFrame:
    filtered = frame.copy()
...
def run_from_namespace(args: argparse.Namespace):
    frame, resolved_time_col = load_dataset(args)
    frame = apply_optional_filters(frame, resolved_time_col, args)
    output_candidate = Path(args.output_root)
    output_root = output_candidate if output_candidate.is_absolute() else discover_project_root(__file__) / output_candidate
    ...
    return run_forecasting_diagnostics(frame, config)
