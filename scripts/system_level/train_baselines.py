...
def main() -> None:
    parser = argparse.ArgumentParser(description="Train and forecast baseline system-level models.")
    ...
    forecasts = run_family_training(target, external, config, "baselines")
    output_path = directories["forecasts"] / "system_level_baseline_forecasts.csv"
    write_dataframe(forecasts, output_path)
    print(f"Wrote system-level baseline forecasts to {output_path}")
